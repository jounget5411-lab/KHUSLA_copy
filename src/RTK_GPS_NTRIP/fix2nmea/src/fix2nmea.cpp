 ...
 public:
  ...
  private:
    ...
    void receiveNavSatFix(const sensor_msgs::msg::NavSatFix::SharedPtr navsat_msg)
    {
  	...
  	double abs_lon = fabs(navsat_msg->longitude);
  	int lon_degs = static_cast<int>(abs_lon); // 세 자리 도수
  	double lon_mins = (abs_lon - lon_degs) * 60.0; // 분
	...
	//double alt_geoid = 1.2;
  	...
  	uint8_t
    len = sprintf(buf,
                  "$GNGGA,%02ld%02ld%02ld.%ld,%02d%04.7f,%c,%03d%07.4f,%c,%d,%d,0.63,%0.3f,M,1.2,M,",
                  time.hours(),
                  time.minutes(),
                  time.seconds(),
                  deci_seconds,
